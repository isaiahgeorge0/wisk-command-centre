...
function plainTextToTipTapNodes(text: string) {
    ...
    useImperativeHandle(
      ...
      () => ({
        insertText: (text: string) => {
          ...
          const nodes = plainTextToTipTapNodes(text);
          ...
          // Flush immediately so the next Winston turn sees the insert.
          if (debounceRef.current) {
            clearTimeout(debounceRef.current);
            debounceRef.current = null;
          }
          setSaveState("saving");
          void updateNoteContent(note.id, serialized).then((result) => {
            if (result.success) {
              setSaveState("saved");
        ...
        <div className="flex items-center gap-2 border-b border-border/60 px-3 py-2">
          ...
          {onToggleProjectProposal ? (
            <Button
              ...
              variant="ghost"
              size="sm"
              onClick={onToggleProjectProposal}
              aria-pressed={proposalOpen}
              aria-label="Find projects and tasks"
              className={cn(
                "gap-1.5 text-xs",
                proposalOpen
            ...
            >
              ...
              <span className="hidden sm:inline">Find projects & tasks</span>
